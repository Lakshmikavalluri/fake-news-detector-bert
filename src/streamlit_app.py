...
with right:
    ...
    if not analyze:
        st.markdown("""
        <div class="card">
            <p style="color:#6b7280;">Results will appear here after you enter news text.</p>
        </div>
        """, unsafe_allow_html=True)
    else:
        ...
        st.markdown(
            ...
            ⚠️ Note: BERT predictions reflect patterns learned from training data. Always verify critical news from trusted sources.
            </p>
            </div>
            """,
            unsafe_allow_html=True,
...
with st.sidebar:
    ...
    st.markdown("---")
    st.write("Paste any long sentence of news article to test its authenticity.")
